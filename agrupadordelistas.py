...
repo_urls = [
    ...
    "https://github.com/gratinomaster/1/raw/refs/heads/main/lista2.M3U",
            ...
            if url.lower().endswith(('.m3u', '.m3u8')) or '#EXTM3U' in response.text:
                ...
                try:
                    ...
                    m3u_files = [
                        ...
                        if content.get("name", "").lower().endswith(('.m3u', '.m3u8'))
                    ]
                    ...
                    print(f"  Erro ao processar JSON de {url}, tratando como M3U direto")
                    filename = url.split("/")[-1]
                    lists.append((filename, response.text))
...
def is_simple_extm3u_header(line):
    """Verifica se é um cabeçalho #EXTM3U simples"""
    line = line.strip()
...
with open(output_file, "w", encoding="utf-8") as f:
    for list_name, list_content in lists:
        ...
        for i in range(start_idx, len(lines)):
            ...
            if line_count >= 212:
                print("Limite de 212 linhas atingido")
                break
...
print("URLs de EPG encontradas e preservadas:")
...
url_archive = "https://archive.org/details/television?query=cbs&sort=date&tab=collection"
...
def limitar_arquivo_m3u(arquivo_original, arquivo_saida, limite_linhas=99999):
    try:
...
print(f"\nArquivo {output_file} criado sem limite de linhas")
print("URLs de EPG encontradas e preservadas:")
for epg_url in epg_urls:
    print(f"  - {epg_url}")
